...
const Form = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(
        "service_83q5tld", // <-- replace with your EmailJS service ID
        "template_hzhknus", // <-- replace with your template ID
        form.current,
        "9JRObDWNnmsFWFV0U" // <-- replace with your EmailJS public key
      )
      .then(
        () => {
    ...
    <div>
      ...
      <div className="mx-2">
        <form ref={form} onSubmit={sendEmail} className="flex flex-col gap-4 mt-4">
          <input type="text" name="user_name" placeholder="Name*" className={commonClass} required />
          <input type="email" name="user_email" placeholder="Email*" className={commonClass} required />
          <input type="text" name="user_location" placeholder="Location*" className={commonClass} required />

          <div className="flex max-xs:flex-col max-xs:gap-4">
            <input type="text" name="user_budget" placeholder="Budget*" className={`${commonClass} xs:w-[50%] me-5`} required />
          ...
          <button
            ...
            disabled={isSending}
          >
            {isSending ? "Sending..." : <>Submit {telegramSVG}</>}
          </button>
